...
class PotAnalysis(models.Model):
	...
	def init(self):
		...
		self.env.cr.execute("""create or replace view pot_analysis as (
			...
			store.state_id as state_id,
			slp.partner_id as partner_id,
			slp.mid as mid,
			slp.tid as tid
			from stock_location_pot slp
			left join stock_location sl on slp.location_id=sl.id
			left join res_partner store on store.id=sl.partner_id
			left join res_partner merchant on merchant.id=store.parent_id
			left join res_partner institution on institution.id=merchant.institution
			left join res_country_state as state on store.state_id= state.id
			group by slp.id,sl.id,institution.id,merchant.id,store.id,store.state_id,slp.partner_id,slp.mid,slp.tid 
			)
